...
def subnet_name(server):
    result = repatter.match(server)
    res = result.group(1) if result != None else ""
    return res
...
class ServerManager():
    ...
    def output_log(self):
        ...
        return f'日時：{self.log_datetime.strftime("%Y/%m/%d %H:%M:%S")}, サブネット：{self.server}, 故障期間(秒)：{error_term_str}'
...
if __name__ == "__main__":
    ...
    f = open('q4.txt', 'x', encoding='UTF-8')
